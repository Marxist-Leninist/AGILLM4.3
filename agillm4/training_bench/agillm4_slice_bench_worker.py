...
def block_kwargs(runtime: Any, cfg: dict[str, Any], rargs: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "moe_shared_experts": int(cfg.get("moe_shared_experts", getattr(runtime, "DEFAULT_MOE_SHARED_EXPERTS", 0))),
        "moe_shared_mlp_mult": int(cfg.get("moe_shared_mlp_mult", getattr(runtime, "DEFAULT_MOE_SHARED_MLP_MULT", 0))),
        "tie_kv": bool(cfg.get("tie_kv", getattr(runtime, "DEFAULT_TIE_KV", False))),
    }
...
def main() -> int:
    ...
    ap.add_argument("--worker-id", default="", help="optional public join worker/node id for result metadata")
    args = ap.parse_args()
